 ...
 * Generate instant playlist from mood or free-form text using AudioMuse-AI.
 * Requires Jellyfin + AudioMuse-AI to be configured.
 */
router.post("/audiomuse/instant", async (req, res) => {
    try {
        ...
        const prompt = mood && VALID_MOODS.includes(mood)
...
router.get("/audiomuse/status", async (req, res) => {
    try {
        const { getAudioMuseConfig } = await import(
            "../services/audioMuseService"
        );
...
/** Helper to format resolved tracks for API response */
async function resolveAndFormatTracks(itemIds: string[]) {
...
router.get("/audiomuse/similar-tracks", async (req, res) => {
    try {
        const cfg = await getJellyfinConfig();
        if (!cfg) {
            return res.status(400).json({ error: "Jellyfin must be configured" });
        }

        const trackId = req.query.trackId as string;
        const n = Math.min(Math.max(1, parseInt(req.query.n as string) || 20), 100);

        if (!trackId || !trackId.startsWith("jellyfin:")) {
            return res.status(400).json({ error: "trackId (jellyfin:xxx) required" });
        }
        ...
        const formatted = await resolveAndFormatTracks(result.tracks.map((t) => t.itemId));
...
router.get("/audiomuse/similar-artists", async (req, res) => {
    try {
        ...
        const n = Math.min(Math.max(1, parseInt(req.query.n as string) || 10), 50);

        const query = artistId || artist;
        if (!query) {
            return res.status(400).json({ error: "artistId or artist required" });
        }
...
router.post("/audiomuse/alchemy", async (req, res) => {
    try {
        ...
        const alchemyItems = items.map((i) => ({
            id: i.id,
            op: (i.op?.toUpperCase() === "SUBTRACT" ? "SUBTRACT" : "ADD") as "ADD" | "SUBTRACT",
            type: (i.type === "artist" ? "artist" : "song") as "song" | "artist",
        }));

        const result = await runAlchemy(alchemyItems, Math.min(n ?? 100, 200));
        if (result.error) {
            return res.status(400).json({ error: result.error });
        }
        if (result.itemIds.length === 0) {
...
router.post("/audiomuse/save-playlist", async (req, res) => {
    try {
        ...
        if (!name?.trim() || !Array.isArray(trackIds) || trackIds.length === 0) {
            return res.status(400).json({ error: "name and trackIds required" });
        }

        const result = await createPlaylistViaAudioMuse(name.trim(), trackIds);
        if (!result.success) {
            return res.status(400).json({ error: result.error });
        }
